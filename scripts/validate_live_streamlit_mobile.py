...
def _assert_no_document_overflow(page, route: str) -> None:
    ...
    for frame in page.frames:
        ...
        if viewport and widest > viewport + 8:
            failures.append(f"frame={frame.url} viewport={viewport} scrollWidth={widest}")
    if checked == 0:
        raise AssertionError(f"/{route} had no PropWar frame available for overflow audit")
    if failures:
        raise AssertionError(f"/{route} has document-level horizontal overflow: {'; '.join(failures)}")
...
def _exercise_expander(page, route: str, label: str, expected_tokens: tuple[str, ...]) -> None:
    ...
    for frame in page.frames:
        try:
            target = frame.get_by_text(label, exact=True)
    ...
    if missing:
        raise AssertionError(f"/{route} mobile control {label!r} missing fields {missing!r}")
    ...
    with sync_playwright() as p:
        ...
        for route, headings in PUBLIC_ROUTES.items():
            try:
                ...
                if expected_data_label and expected_data_label not in body:

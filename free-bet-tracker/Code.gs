...
function listPromos_() {
  ...
  rows.forEach(r => { if (r[0] && !r[7]) { r[7] = Utilities.getUuid(); idsChanged = true; } });
  if (idsChanged) sh.getRange(START_ROW, 8, rows.length, 1).setValues(rows.map(r => [r[7] || '']));
  return rows.map(r => {
    if (!r[0]) return null;
    const expires = r[2] instanceof Date ? r[2].toISOString() : String(r[2] || '');
    return {id: String(r[7]), book: String(r[0] || ''), value: Number(r[1] || 0), expires, used: Boolean(r[4]), promo: String(r[5] || ''), notes: String(r[6] || '')};
  }).filter(Boolean);
}
...
function addPromo_(p) {
  ...
  sh.getRange(row, 1, 1, 8).setValues([[String(p.book || ''), Number(p.value || 0), expires, '', String(p.used) === 'true', String(p.promo || ''), String(p.notes || ''), String(p.id || Utilities.getUuid())]]);

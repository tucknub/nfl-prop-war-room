...
| Season | Sleeper league ID | Status | Notes |
|---|---|---|---|
| 2024 | `1041254319653720064` | historical | Backfill audit confirmed draft, matchup, and transaction history. |
| 2025 | `1237498478561603584` | complete / historical | Direct predecessor of current 2026 league. |
...
- Playoffs start Week 15.
- Trade deadline Week 12.
- The current **league settings do not expose a verified season-long QB roster maximum**.
...
- Draft position limits are enabled (`enforce_position_limits = 1`).
- The draft QB position limit is 3 (`position_limit_qb = 3`).
- This is classified as a **draft constraint**, not a proven in-season roster maximum.
...
3. Draft-only position limits belong to normalized draft state and must not be promoted into season-long league rules unless the league resource independently proves the same restriction.
4. Two leagues can both be Full PPR/1QB yet still require materially different player valuation because of team count, starting WR/FLEX depth, FAAB budget, defense usage, bench depth, and other rules.
5. Draft settings must come from the draft resource; both verified Sleeper leagues currently expose misleading `settings.draft_rounds = 3` values relative to their actual draft objects.
...
| Playoff teams | 6 | 6 |
| Draft rounds | 16 | 15 |
| Draft QB limit | none verified | 3 |
...
Draft-only constraints belong to draft-state fingerprinting/versioning rather than the season rules fingerprint unless the provider also exposes them as league rules.

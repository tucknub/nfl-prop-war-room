...
def test_redevelopment_loader_rejects_post_2021_request(tmp_path):
    path = tmp_path / "canonical.csv"
    pd.DataFrame({"season": [2018, 2022], "value": [1, 2]}).to_csv(path, index=False)
    with pytest.raises(ValueError, match="2018.*2021"):
        load_canonical_seasons(str(path), seasons=[2022])
